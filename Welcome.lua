...
local function description_rules(msg, nama)
   ...
   if data[tostring(msg.to.id)] then
      ...
      local sambutan = "هلا🍷🌝 "..nama.."\n نورت/ورده كمل قرأه '"..string.gsub(msg.to.print_name, "_", " ").."'\n'
      local text = sambutan.."\n لاضهار القوانين /القوانين 🙏🏿   "
      local text = text.."\n للخروج من المجموعة  /مغادره☹"
      local text = text..""
      local text = text.."\n يرجى منك الدخول الى قناة السورس ☹"
      local text = text..""
      local text = text.."\n اي استفسار تعال للمطور @z557z واذا محظور ادخل هنا @istaraliraq_bot ☹"
      local text = text..""
      local receiver = get_receiver(msg)
